...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // Speech recognition
  startSpeechRecognition: (options) => ipcRenderer.invoke('start-speech-recognition', options),
  stopSpeechRecognition: () => ipcRenderer.invoke('stop-speech-recognition'),
  sendAudioChunk: (buffer, level, captureId) => ipcRenderer.send('audio-chunk', { buffer, level, captureId }),
  getSpeechCaptureConfig: () => ipcRenderer.invoke('get-speech-capture-config'),
  audioCaptureStopped: (captureId) => ipcRenderer.send('audio-capture-stopped', { captureId }),
  audioCaptureError: (error, captureId) => ipcRenderer.send('audio-capture-error', { error, captureId }),
  onStopAudioCapture: (callback) => ipcRenderer.on('stop-audio-capture', callback),
  onSpeechLevel: (callback) => ipcRenderer.on('speech-level', callback),
  getSpeechAvailability: () => ipcRenderer.invoke('get-speech-availability'),
  
  // Window management
  minimizeToDot: () => ipcRenderer.invoke('minimize-to-dot'),
  restoreFromDot: () => ipcRenderer.invoke('restore-from-dot'),
  moveMiniDot: (phase) => ipcRenderer.invoke('move-mini-dot', phase),
  ...
  getAvailableSkills: () => ipcRenderer.invoke('get-available-skills'),
  ...
  onTranscriptionLlmResponseError: (callback) => ipcRenderer.on('transcription-llm-response-error', callback),
