...
const { skills, defaultSkill } = require('./src/core/skills');
...
class PromptLoader {
  constructor() {
    ...
    this.skillsRequiringProgrammingLanguage = skills.filter(skill => skill.requiresProgrammingLanguage).map(skill => skill.id);
  }
    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith('.md')) {
          ...
          if (!skills.some(skill => skill.id === skillName)) continue;
  ...
  getSkillPrompt(skillName, programmingLanguage = null) {
    ...
    if (programmingLanguage && programmingLanguage !== 'auto' && this.requiresProgrammingLanguage(normalizedSkillName)) {
...
- Avoid unnecessary verbosity; focus on correctness, clarity, and efficiency.`;
        break;
      default:
        languageInjection = `\n\n## PREFERRED PROGRAMMING LANGUAGE: ${languageUpper}\nPrefer ${languageTitle} when no language is specified. Follow the user's explicit request and the language of their existing code when provided. Tag every code block with its actual language.`;
    }
  ...
  normalizeSkillName(skillName) {
    if (typeof skillName !== 'string' || !skillName.trim()) return defaultSkill;
  ...
  getAvailableSkills() {
    ...
    return skills.filter(skill => this.prompts.has(skill.id)).map(skill => skill.id);
  }

  getSkillCatalog() {
    const available = this.getAvailableSkills();
    return skills.filter(skill => available.includes(skill.id)).map(skill => ({ ...skill }));
  }
