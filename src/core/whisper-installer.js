...
const INSTALL_TIMEOUT_MS = 1200000; // PyTorch installation can be slow on a cold cache
...
class WhisperInstaller {
   ...
   */
  get modelDir() {
    if (process.env.WHISPER_MODEL_DIR && path.isAbsolute(process.env.WHISPER_MODEL_DIR)) {
      return process.env.WHISPER_MODEL_DIR;
    }
  ...
  async install({ onProgress } = {}) {
    ...
    const pipResult = await this.runExec(vp.python, ['-m', 'pip', 'install', '--upgrade', 'openai-whisper', 'faster-whisper'], {
    ...
    const verify = await this._probe([vp.python, '-m', 'whisper']);
    if (!verify.ok) {
      ...
      case 'win32':
        return {
          ...
          steps: [
            ...
            'First transcription downloads the <code>base</code> model (~142 MB).',
          ],
        };
      case 'darwin':
        return {
  ...
  async _probe(candidate) {
    ...
    if (moduleIndex > 0 && candidate[moduleIndex + 1] === 'whisper') {
      const result = await this.runExec(cmd, [
        ...candidate.slice(1, moduleIndex), '-c',
        'import importlib.util,sys; print("optimized" if importlib.util.find_spec("faster_whisper") else "standard"); sys.exit(0 if importlib.util.find_spec("whisper") else 1)'
      ]);
      return { ok: result.ok, version: null, optimized: (result.stdout || '').includes('optimized') };
    }
  ...
  async downloadModel(modelName = 'base', { onProgress } = {}) {
    const log = (line) => {
      if (typeof onProgress === 'function' && line) {
    ...
    let workerPath = path.resolve(__dirname, '../../scripts/whisper_worker.py');
    const unpackedPath = workerPath.replace(/app\.asar([\\/])/, 'app.asar.unpacked$1');
    if (fs.existsSync(unpackedPath)) workerPath = unpackedPath;
    const loadResult = await this.runExec(pythonCmd, [
      workerPath, '--prepare-model', modelName, '--model-dir', downloadRoot
    ], {
      timeout: 600000,
      onProgress: log,
    });
    ...
    let engine;
    try { engine = JSON.parse(loadResult.stdout.trim().split(/\r?\n/).pop()).engine; } catch (_) {}
    const modelPath = engine === 'faster' ? downloadRoot : this._getModelPath(modelName);
    log(`✓ Model ${modelName} ready at ${modelPath}`);
    return { ok: true, message: `Model ${modelName} downloaded successfully`, path: modelPath, engine };
  }
