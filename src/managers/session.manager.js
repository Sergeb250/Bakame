...
const { defaultSkill } = require('../core/skills');
...
class SessionManager {
  constructor() {
    ...
    this.currentSkill = defaultSkill;
  ...
  addConversationEvent({ role, content, action = null, metadata = {} }) {
    const event = this.createConversationEvent({
      role,
      content,
      skill: metadata.skill || this.currentSkill,
   ...
   */
  addUserInput(text, source = 'chat', skill = this.currentSkill) {
   ...
   */
  getConversationHistory(maxEntries = 20, skill = this.currentSkill) {
    // Get recent conversation events (excluding system initialization)
    const conversationEvents = this.sessionMemory
      .filter(event => event.skill === skill && (event.role !== 'system' || !event.metadata?.isInitialization))
      .slice(-maxEntries);
