...
const MiniOverlay = require('./mini-overlay');

class WindowManager {
  constructor() {
    this.windows = new Map();
    this.pendingWindows = new Map();
    this.miniOverlay = new MiniOverlay(this);
    ...
    this.boundWindowsPosition = null;
    this.appliedMainPosition = null;
    this.hasUserPosition = false;
    
    this.windowConfigs = {
      mini: { width: 40, height: 40, file: 'mini-overlay.html', title: 'Restore Bakame' },
      main: {
        width: 560,
        height: 35,
        useContentSize: true,
  ...
  async createMainWindow(options = {}) {
    ...
    // show â€” it persists into the future showOnCurrentDesktop call.
    if (process.platform === 'darwin') {
      try {
    ...
    // `showMainWindow()` â€” callers control timing via the flag below.
    if (autoShow) {
      // Wait for app to fully initialize and detect current desktop
      setTimeout(() => {
  ...
  createWindow(type, showOnCreate = false) {
    if (type === 'chat') type = 'llmResponse';
    const existing = this.windows.get(type);
    if (existing && !existing.isDestroyed()) return Promise.resolve(existing);
    if (this.pendingWindows.has(type)) return this.pendingWindows.get(type);
    const pending = this.createProtectedWindow(type, showOnCreate).then(window => {
      this.windows.set(type, window);
      return window;
    }).finally(() => this.pendingWindows.delete(type));
    this.pendingWindows.set(type, pending);
    return pending;
  }

  async createProtectedWindow(type, showOnCreate = false) {
    if (require('electron').app.bakameQuitting) throw new Error('Application is shutting down');
    ...
    const baseOptions = {
      ...
      webPreferences: {
        ...
        disableDialogs: true, // Enable DevTools for debugging
      },
    ...
    if (type === 'mini') {
      browserWindowOptions = { ...baseOptions, frame: false, transparent: true, backgroundColor: '#00000000',
      ...
      // First-run onboarding wizard â€” same frameless/panel style as
  ...
  resizable: true,
    ...
    maxWidth: this.windowConfigs.main.width,
      ...
      // The shared conversation keeps its input controls reachable.
      browserWindowOptions = {
        ...baseOptions,
        minWidth: 400,
        minHeight: 320,
  ...
  const window = new BrowserWindow(browserWindowOptions);
    ...
    window.once('closed', () => {
      ...
      if (type === 'onboarding') require('electron').app.emit('bakame-onboarding-closed');
    });
    window.webContents.on('page-title-updated', event => event.preventDefault());
    if (type === 'main') {
      window.setClosable(true);
      window.on('close', event => {
        if (require('electron').app.bakameQuitting) return;
        event.preventDefault();
        require('electron').app.emit('bakame-quit');
      });
    }
  ...
  // Load the HTML file
    ...
    if (require('electron').app.bakameQuitting) {
  ...
  // Position the window
    ...
    if (type === 'main') window.on('move', () => this.handleMainWindowMove());
    
  // Apply simplified stealth measures
    if (type === 'mini') {
      window.setAlwaysOnTop(true);
      // Crossing displays can change the outer size by a pixel due to DPI rounding.
      const keepDotOnScreen = () => { if (this.miniOverlay.position) this.miniOverlay.place(this.miniOverlay.position); };
      window.on('resize', keepDotOnScreen);
      window.on('move', keepDotOnScreen);
      window.on('closed', () => { if (this.miniOverlay.minimized && !require('electron').app.bakameQuitting) this.miniOverlay.restore(); });
    } else this.applyStealthMeasures(window, type);
    
  // Initialize interaction mode based on current state for ALL windows
    if (this.isInteractive || type === 'mini') {
  ...
  // Retain the chosen toolbar position when answers arrive or either window resizes.
  positionBoundWindows() {
    const mainWindow = this.windows.get('main');
    const llmWindow = this.bindWindows ? this.windows.get('llmResponse') : null;
    if (!mainWindow || mainWindow.isDestroyed()) return;
    const display = this.currentDisplay || screen.getPrimaryDisplay();
    const response = llmWindow && !llmWindow.isDestroyed() ? llmWindow : null;
    const layout = getOverlayLayout({
      anchor: this.boundWindowsPosition,
      mainSize: mainWindow.getSize(),
      responseSize: response?.getSize(),
      workArea: display.workArea,
      gap: this.windowGap
    });
    // Set this before moving: Electron can emit move synchronously or later.
    this.appliedMainPosition = layout.main;
    if (!this.boundWindowsPosition) this.boundWindowsPosition = { ...layout.main };
    const [x, y] = mainWindow.getPosition();
    if (x !== layout.main.x || y !== layout.main.y) mainWindow.setPosition(layout.main.x, layout.main.y);
    if (response) {
      const [rx, ry] = response.getPosition();
      if (rx !== layout.response.x || ry !== layout.response.y) response.setPosition(layout.response.x, layout.response.y);
    }
  }
  ...
  handleMainWindowMove() {
    ...
    if (!mainWindow || mainWindow.isDestroyed()) return;
    const [x, y] = mainWindow.getPosition();
    if (this.appliedMainPosition?.x === x && this.appliedMainPosition?.y === y) return;
    this.boundWindowsPosition = { x, y };
    this.hasUserPosition = true;
    this.currentDisplay = screen.getDisplayMatching(mainWindow.getBounds());
    this.positionBoundWindows();
  }

  moveBoundWindows(deltaX, deltaY) {
    if (this.miniOverlay.minimized) {
      if (!Number.isFinite(deltaX) || !Number.isFinite(deltaY)) return false;
      const { x, y } = this.miniOverlay.position;
      this.miniOverlay.moved = true;
      this.miniOverlay.place({ x: x + deltaX, y: y + deltaY });
      return true;
    }
    const mainWindow = this.windows.get('main');
    if (!mainWindow || mainWindow.isDestroyed() || !Number.isFinite(deltaX) || !Number.isFinite(deltaY)) return false;
    const [x, y] = mainWindow.getPosition();
    this.boundWindowsPosition = { x: x + Math.round(deltaX), y: y + Math.round(deltaY) };
    this.hasUserPosition = true;
    this.currentDisplay = screen.getDisplayMatching(mainWindow.getBounds());
    this.positionBoundWindows();
    return true;
  ...
  showOnCurrentDesktop(win) {
    if (!win || win.isDestroyed() || this.miniOverlay.keepHidden(win)) return;
    if (win === this.windows.get('mini')) {
      if (this.miniOverlay.minimized) win.showInactive();
      return;
    }
    ...
    if (process.platform === 'darwin') {
      ...
      setTimeout(() => {
        if (win.isDestroyed() || this.miniOverlay.keepHidden(win)) return;
        win.show();
        win.focus();
        setMacOSAlwaysOnTop();
  ...
  switchToWindow(windowType) {
    if (windowType === 'chat') windowType = 'llmResponse';
    ...
    if (windowType !== 'mini') this.miniOverlay.restore();
  ...
  showAllWindows() {
    if (this.miniOverlay.minimized) return this.miniOverlay.restore();
    if (this.isScreenBeingShared) {
      return;
    }

    this.windows.forEach((window, type) => {
      if (type !== 'llmResponse' && type !== 'mini') { // Conversation opens when requested
        this.showOnCurrentDesktop(window);
      }
    });
  ...
  toggleVisibility() {
    if (this.miniOverlay.minimized) { this.miniOverlay.restore(); return true; }
    if (this.isScreenBeingShared) {
  ...
  getWindow(type) {
    return this.windows.get(type === 'chat' ? 'llmResponse' : type);
  }
  ...
  getWindowStats() {
    ...
    return {
      ...
      isMinimizedToDot: this.miniOverlay.minimized,
      isVisible: this.isVisible,
      isScreenBeingShared: this.isScreenBeingShared,
  ...
  destroyAllWindows() {
    this.miniOverlay.minimized = false;
    // Include windows still loading and not yet registered in the window map.
  ...
  handleDisplayChange() {
    setTimeout(() => {
      if (this.miniOverlay.position) this.miniOverlay.place(this.miniOverlay.position);
      const mainWindow = this.windows.get('main');
      if (mainWindow && !mainWindow.isDestroyed()) this.currentDisplay = screen.getDisplayMatching(mainWindow.getBounds());
      this.moveWindowsToActiveScreen();
    }, 500);
  }

  trackActiveScreen() {
    if (this.isScreenBeingShared || this.hasUserPosition || this.miniOverlay.minimized) return;
  ...
  showChatWindow() {
    const chatWindow = this.windows.get('llmResponse');
  ...
  handleRecordingStarted(captureConfig) {
    this.isRecording = true;
    this.showChatWindow();
    // Notify all windows about recording state
    this.broadcastToAllWindows('recording-started', captureConfig);
