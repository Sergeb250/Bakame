...
class LLMService {
  ...
  async processImageWithSkill(imageBuffer, mimeType, activeSkill, sessionMemory = [], programmingLanguage = null) {
    ...
    try {
      ...
      const finalResponse = activeSkill === 'dsa' && programmingLanguage && programmingLanguage !== 'auto'
  ...
  formatImageInstruction(activeSkill, programmingLanguage) {
    const skill = promptLoader.getSkillCatalog().find(item => item.id === activeSkill);
    if (activeSkill === 'exam') {
      return 'Identify every question and subpart in this screenshot and answer each one in reading order. Preserve question numbers and option labels. Read any accompanying passage, diagram, or table. Give concise answers and essential working where needed. Flag unreadable or missing details instead of guessing. If there are no questions, ask for an image containing the questions.';
    }
    if (activeSkill === 'dsa') {
      const langNote = programmingLanguage && programmingLanguage !== 'auto'
        ? ` Use only ${programmingLanguage.toUpperCase()} for any code.` : '';
      return `Analyze the algorithm problem in this image. Extract the problem concisely and provide a solution with explanation and final code.${langNote}`;
    }
    return `Use ${skill?.name || 'General Assistant'} mode to help with the task in this screenshot. Read the visible content and follow any explicit request. If no request is visible, summarize what is shown and offer useful next steps appropriate to this mode. State when details are unreadable.`;
  }
  ...
  buildGeminiRequest(text, activeSkill, sessionMemory, programmingLanguage) {
    ...
    if (sessionManager && typeof sessionManager.getConversationHistory === 'function') {
      const conversationHistory = sessionManager.getConversationHistory(15, activeSkill);
  ...
  buildGeminiRequestWithHistory(text, activeSkill, conversationHistory, skillContext, programmingLanguage) {
    ...
    if (last?.role === 'user' && last.parts[0].text === text.trim()) {
  ...
  buildIntelligentTranscriptionRequest(text, activeSkill, sessionMemory, programmingLanguage) {
    ...
    if (sessionManager && typeof sessionManager.getConversationHistory === 'function') {
      const conversationHistory = sessionManager.getConversationHistory(10, activeSkill);
      const skillContext = sessionManager.getSkillContext(activeSkill, programmingLanguage);
  ...
  // Voice input uses the same skill instructions as typed messages.
  const intelligentPrompt = this.getIntelligentTranscriptionPrompt(activeSkill, programmingLanguage);
  ...
  getIntelligentTranscriptionPrompt(activeSkill, programmingLanguage) {
    const skillPrompt = promptLoader.getSkillPrompt(activeSkill, programmingLanguage)
      || promptLoader.getSkillPrompt('general');
    return `${skillPrompt}

## Voice input
The user's message was transcribed from speech. Answer the actual request using
this skill's instructions. Interpret small transcription errors using context.
For a greeting, acknowledge it briefly. For a task, provide useful help rather
than rejecting it as unrelated to the skill. Ask a focused clarification if the
transcription is too ambiguous to act on. Do not invent missing spoken details.`;
  }
  ...
  generateFallbackResponse(text, activeSkill) {
    return {
      response: 'The AI provider could not answer this request. Check your Gemini API key and connection in Settings, then try again.',
      metadata: {
        skill: activeSkill,
        processingTime: 0,
        requestId: this.requestCount,
  ...
  generateIntelligentFallbackResponse(text, activeSkill) {
    const result = this.generateFallbackResponse(text, activeSkill);
    result.metadata.isTranscriptionResponse = true;
    return result;
  }
