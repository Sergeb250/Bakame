...
class SpeechService extends EventEmitter {
  constructor() {
    ...
    this.recording = null;
    this.recognitionStartTimer = null;
  ...
  _initializeAzureClient() {
    try {
      ...
      if (!this._supportsRendererCapture() && (!recorder || typeof recorder.record !== 'function')) {
  ...
  startRecording() {
    try {
      ...
      if (this.isProcessingAudio) {
        ...
        return { success: false, error: 'Please wait for the current transcription to finish' };
      }
  ...
  _startAzureRecording() {
    ...
    this._cleanup();
    this.captureId++;
    this.useRendererCapture = this._supportsRendererCapture();
    // Web Audio supplies 16 kHz, 16-bit mono PCM, the SDK's default format.
    ...
    recognizer.startContinuousRecognitionAsync(
      () => {
        if (this.recognizer !== recognizer || !this.isRecording) return;
  ...
  _startWhisperRecording() {
    ...
    this._previewAudioMs = 0;
    this.useRendererCapture = this._supportsRendererCapture();
    const queue = new TranscriptionQueue((buffer, options) => {
      if (this.transcriptionQueue !== queue) return '';
      return this._transcribeWhisperBuffer(buffer, options);
    });
    this.transcriptionQueue = queue;
    queue.on('transcription', (text, metadata) => {
      if (this.transcriptionQueue !== queue) return;
      this.emit('interim-transcription', '', { ...metadata, final: true });
      const clean = (text || '').trim();
      if (clean && !this._isHallucinatedTranscript(clean)) this.emit('transcription', clean, metadata);
    });
    queue.on('partial', (text, metadata) => {
      if (this.transcriptionQueue === queue && this.isRecording) {
        const clean = (text || '').trim();
        if (clean && !this._isHallucinatedTranscript(clean)) this.emit('interim-transcription', clean, metadata);
      }
    });
    queue.on('failure', (error, metadata) => {
      if (this.transcriptionQueue !== queue) return;
      if (metadata.preview) logger.warn('Whisper preview failed', { error: error.message });
      else this.emit('error', `Whisper transcription failed: ${error.message}`);
    });
    queue.on('backlog', milliseconds => {
      if (this.transcriptionQueue !== queue) return;
      this.pendingAudioMs = milliseconds;
      if (milliseconds > 60000 && this.isRecording) {
        this.emit('status', 'Transcription is falling behind. Listening is paused; choose a smaller Whisper model.');
        this.stopRecording();
      }
    });
    this.whisperWorker.setKeepWarm(true);
    this.emit('recording-started', this.getCaptureConfig());
    this.emit('status', 'Loading Whisper for live listening...');
    if (this.whisperWorker.isConfigured()) {
      this.whisperWorker.warmup({
        model: this._getWhisperModel(), modelDir: this._getWhisperModelDir(), device: this._getWhisperDevice()
      }).then(() => {
        if (this.transcriptionQueue === queue && this.isRecording) this.emit('status', 'Listening - Whisper ready');
      }).catch(error => {
        if (this.transcriptionQueue === queue) this.emit('error', `Whisper warmup failed: ${error.message}`);
      });
    }
    if (!this.useRendererCapture) this._startMicrophoneCapture();
    if (!this._isManualCaptureMode()) this._startSegmentWatchdog();
  }

  _supportsRendererCapture() {
  ...
  getCaptureConfig() {
    return {
      ...
      captureMode: this._getWhisperCaptureMode(),
      systemAudioSupported: process.platform === 'win32'
    };
  }

  getAudioSource() {
    if (process.platform !== 'win32') return 'microphone';
    const source = process.env.WHISPER_AUDIO_SOURCE;
    if (['microphone', 'system', 'both'].includes(source)) return source;
    return process.platform === 'win32' ? 'both' : 'microphone';
  }

  _maybePreviewWhisper() {
    if (this._isManualCaptureMode() || !this.whisperWorker.isConfigured() || !this.transcriptionQueue) return;
    const audioMs = this.segmentBytes / 32;
    if (this.vadSpeechMs < 1000 || audioMs - this._previewAudioMs < 1500) return;
    this._previewAudioMs = audioMs;
    this.transcriptionQueue.previewLatest(Buffer.concat(this.segmentBuffers, this.segmentBytes), this._utteranceId);
  }
  ...
  stopRecording() {
    ...
    if (this.provider === 'azure' && this.recognizer) {
      ...
      this.emit('status', 'Finishing speech recognition...');
      try {
  ...
  async _finalizeWhisperStop({ captureAlreadyStopped = false } = {}) {
    ...
    try {
      ...
      this._cleanup();
      this.isProcessingAudio = false;
      this.whisperWorker.setKeepWarm(false);
  ...
  getStatus() {
    return {
      ...
      isProcessingAudio: this.isProcessingAudio,
      captureId: this.captureId,
      pendingAudioMs: this.pendingAudioMs || 0,
      audioSource: this.getAudioSource(),
  ...
  _getWhisperModelDir() {
    ...
    // with the stable directory the installer downloads weights into,
  ...
  _getUserDataModelDir() {
    try {
      ...
      return path.join(app.isPackaged ? app.getPath('userData') : path.resolve(__dirname, '../..'), '.whisper-models');
    } catch (_) {
      return '';
    }
  }
  ...
  _getWhisperPythonPath() {
    ...
    if (fs.existsSync(siblingPython)) return siblingPython;
    const userDataCandidate = this._getUserDataWhisperCandidate();
    return userDataCandidate && fs.existsSync(userDataCandidate.command) ? userDataCandidate.command : null;
  }
  ...
  _resolveWhisperCommand() {
    ...
    const localPython = path.resolve(__dirname, '../../.venv-whisper',
      process.platform === 'win32' ? 'Scripts/python.exe' : 'bin/python');
    if (fs.existsSync(localPython)) candidates.push({ command: localPython, baseArgs: ['-m', 'whisper'], source: 'project venv' });
  ...
  _handleAudioChunk(chunk) {
    ...
    if (this.provider === 'azure' && this.pushStream) {
      try {
        // The SDK expects an ArrayBuffer. Respect Buffer offsets so pooled
        // memory outside this PCM chunk is never included in the stream.
        const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
  ...
  async _flushWhisperSegment({ final }) {
    const queue = this.transcriptionQueue;
    if (!queue) return;
    // Detach every utterance immediately, including while Whisper is busy.
    // Stop waits for this queue before clearing the service's buffers.
    if (this.segmentBytes) {
      const audio = Buffer.concat(this.segmentBuffers, this.segmentBytes);
      this.segmentBuffers = [];
      this.segmentBytes = 0;
      this._previewAudioMs = 0;
      queue.enqueue(audio, this._utteranceId++);
    }
    if (final) await queue.drain();
  }
  ...
  async _transcribeWhisperBuffer(audioBuffer, options = {}) {
    ...
    try {
      ...
      return await this._transcribeWhisperFile(audioFilePath, options);
    } finally {
      this._removeTempDir(tempDir);
    }
  }

  async _transcribeWhisperFile(audioFilePath, options = {}) {
    if (!this.whisperCommand) {
