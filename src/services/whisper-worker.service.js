...
class WhisperWorkerService {
  ...
  setKeepWarm(enabled) {
    this.keepWarm = enabled;
    if (enabled) this._clearIdleTimer();
  }
  ...
  _ensureProcess() {
    ...
    lines.on('line', (line) => {
      if (this.process === child) this._handleLine(line);
    });
  ...
  _handleLine(line) {
    ...
    if (message.ok) {
      if (message.warmed) this.readyInfo = { ...this.readyInfo, ...message };
