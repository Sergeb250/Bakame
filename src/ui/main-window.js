...
class MainWindowUI {
    ...
    async init() {
        try {
            ...
            document.addEventListener('protected-ui-size', () => this.resizeWindowToContent());
            document.getElementById('quitButton')?.addEventListener('click', () => window.electronAPI.quit());
    ...
    async loadCurrentSkill() {
        try {
            if (window.electronAPI && window.electronAPI.getSettings) {
                ...
                if (settings && settings.availableSkills) {
                    this.skillCatalog = settings.availableSkills;
                    this.availableSkills = this.skillCatalog.map(skill => skill.id);
                    this.skillSelect.replaceChildren(...this.skillCatalog.map(skill => {
                        const option = new Option(skill.name, skill.id);
                        option.dataset.tooltip = skill.description;
                        return option;
                    }));
                }
    ...
    applyMicVisibility() {
        if (this.micButton) {
            ...
            this.micButton.dataset.tooltip = this.speechAvailable ? 'Start or stop live listening (Alt+R)' : 'Set up Whisper in Settings';
            this.resizeWindowToContent();
        }
    }
    ...
    updateSkillIndicatorState() {
        if (this.skillSelect) this.skillSelect.disabled = !this.isInteractive || !this.skillCatalog.length;
        if (this.languageSelect) this.languageSelect.disabled = !this.isInteractive;
    ...
    resizeWindowToContent() {
        ...
        setTimeout(() => {
            ...
            if (commandTab && window.electronAPI && window.electronAPI.resizeWindow) {
                ...
                for (const popup of document.querySelectorAll('.protected-floating')) {
                    height = Math.max(height, Math.ceil(popup.getBoundingClientRect().bottom + 8));
                }
    ...
    setupElements() {
        document.getElementById('minimizeButton')?.addEventListener('click', async () => {
            ...
            try {
                const result = await window.electronAPI.minimizeToDot();
                if (!result.success) this.showNotification(result.error || 'Could not minimize Bakame.', 'error');
            } catch (error) { this.showNotification(error.message, 'error'); }
        });
        document.getElementById('conversationButton')?.addEventListener('click', () => {
            if (this.isInteractive) window.electronAPI.switchToChat();
        });
        document.getElementById('moveHandle')?.addEventListener('keydown', event => {
            if (this.isInteractive && !event.ctrlKey && !event.metaKey && !event.altKey && !event.shiftKey && ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(event.key)) {
                event.preventDefault();
                event.stopPropagation();
                this.moveWindow(event.key);
            }
        });
    ...
    if (!this.statusDot || !this.skillIndicator || !this.micButton || !this.screenshotButton) {
        ...
        this.skillSelect.addEventListener('change', () => {
            this.selectSkill(this.skillSelect.value);
        });
            ...
            if (this.isInteractive && this.speechAvailable) {
                try {
                    if (this.isRecording) {
                        ...
                        if (result?.success === false) this.showNotification(result.error, 'error');
                    }
                } catch (error) {
    ...
    setupEventListeners() {
        if (window.electronAPI) {
            ...
            window.electronAPI.onRecordingStarted((_event, config) => {
                this.handleRecordingStarted(config);
            });

            window.electronAPI.onStopAudioCapture(async (_event, data) => {
                if (data.captureId !== this.activeCaptureId) return;
                await this.audioCapture?.stop();
                window.electronAPI.audioCaptureStopped(data.captureId);
            });
    ...
    handleLLMResponse(data) {
        ...
        const skillNames = Object.fromEntries(this.skillCatalog.map(item => [item.id, item.name]));
    ...
    async startAudioCapture(config) {
        if (this.activeCaptureId === config.captureId && this.audioCapture?.resources) return;
        if (!config.useRendererCapture) return;
        this.handleRecordingStarted();
        if (!this.audioCapture) {
            this.audioCapture = new window.LiveAudioCapture({
                onAudio: (buffer, level, captureId) => window.electronAPI.sendAudioChunk(buffer, level, captureId),
                onError: (error, captureId) => {
                    window.electronAPI.audioCaptureError(error, captureId);
                    this.showNotification(error, 'error');
                }
            });
        }
        this.activeCaptureId = config.captureId;
        await this.audioCapture.start(config);
    }
    ...
    updateSkillIndicator() {
        ...
        const skill = this.skillCatalog.find(item => item.id === this.currentSkill);
        this.skillSelect.value = this.currentSkill;
        this.skillIndicator.dataset.tooltip = skill?.description || 'Choose an assistant skill';
        const showLanguage = !!skill?.requiresProgrammingLanguage;
        document.getElementById('languageSelector').style.display = showLanguage ? '' : 'none';
        document.getElementById('languageSeparator').style.display = showLanguage ? '' : 'none';
        this.updateSkillIndicatorState();
        this.resizeWindowToContent();
    }
    ...
    async selectSkill(skill) {
        ...
        this.skillSelect.disabled = true;
        try {
            const result = await window.electronAPI.updateActiveSkill(skill);
            if (!result.success) throw new Error(result.error || 'Could not change skill');
            this.handleSkillActivated(result.skill);
        } catch (error) {
    ...
    navigateSkill(direction) {
        if (!this.isInteractive || !this.availableSkills.length) return;
        const currentIndex = this.availableSkills.indexOf(this.currentSkill);
        const nextIndex = (currentIndex + direction + this.availableSkills.length) % this.availableSkills.length;
        return this.selectSkill(this.availableSkills[nextIndex]);
    }
